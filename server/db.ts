...
export const db = {
  ...
  tenants: {
    ...
    findOne: (conditions: any) => {
      ...
      return sql`SELECT * FROM public.tenants WHERE ${sql.unsafe(whereClause)} LIMIT 1`.values(...Object.values(conditions));
    },
    ...
    findAll: (conditions: any = {}) => {
      ...
      const whereClause = keys.map((key, i) => `${key} = $${i + 1}`).join(' AND ');
      return sql`SELECT * FROM public.tenants WHERE ${sql.unsafe(whereClause)} ORDER BY created_at DESC`.values(...Object.values(conditions));
    },

    update: (id: string, data: any) => {
      const keys = Object.keys(data);
      const setClause = keys.map((key, i) => `${key} = $${i + 2}`).join(', ');
      return sql`UPDATE public.tenants SET ${sql.unsafe(setClause)}, updated_at = NOW() WHERE id = $1 RETURNING *`.values(id, ...Object.values(data));
    },
  ...
  wizardSessions: {
    ...
    update: (id: string, data: any) => {
      const keys = Object.keys(data);
      const setClause = keys.map((key, i) => `${key} = $${i + 2}`).join(', ');
      return sql`UPDATE public.wizard_sessions SET ${sql.unsafe(setClause)}, updated_at = NOW() WHERE id = $1 RETURNING *`.values(id, ...Object.values(data));
    },
  ...
  onboardingTasks: {
    ...
    update: (id: string, data: any) => {
      const keys = Object.keys(data);
      const setClause = keys.map((key, i) => `${key} = $${i + 2}`).join(', ');
      return sql`UPDATE public.onboarding_tasks SET ${sql.unsafe(setClause)}, updated_at = NOW() WHERE id = $1 RETURNING *`.values(id, ...Object.values(data));
    },
    ...
    updateByTenantAndType: (tenantId: string, taskType: string, data: any) => {
      ...
      return sql`
        ...
        RETURNING *
      `.values(tenantId, taskType, ...Object.values(data));
    },
  },
  ...
  supportTickets: {
    ...
    update: (id: string, data: any) => {
      const keys = Object.keys(data);
      const setClause = keys.map((key, i) => `${key} = $${i + 2}`).join(', ');
      return sql`UPDATE public.support_tickets SET ${sql.unsafe(setClause)}, updated_at = NOW() WHERE id = $1 RETURNING *`.values(id, ...Object.values(data));
    },
  },
